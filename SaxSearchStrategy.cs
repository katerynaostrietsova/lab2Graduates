...
namespace lab2Graduates.Strategies
{
    public class SaxSearchStrategy : IXmlSearchStrategy
    {
            ...
            string? keyword)
        {
            ...
            // Йдемо по документу послідовно
            while (!reader.EOF)
            {
                if (reader.NodeType == XmlNodeType.Element &&
                    reader.Name == "graduate")
                {
                    // Читаємо поточний <graduate> як XElement
                    var element = (XElement)XNode.ReadFrom(reader);

                    // Розбираємо його в модель Graduate + окремо зберігаємо group
                ...
                {
                    // Якщо це не <graduate>, просто рухаємося далі
                    reader.Read();
                }
            }
        ...
        private static Graduate ReadGraduate(XElement element, out string groupFromXml)
        {
            ...
            // group зберігаємо в окрему змінну (у моделі такої властивості немає)
        ...
        private static bool MatchesAttributeFilter(
            ...
            string? attributeValue)
        {
            // Якщо фільтр по атрибуту не заданий – пропускаємо всіх
            ...
            string? actual = attributeName switch
            {
                ...
                "group" => groupFromXml, // group беремо прямо з XML
        ...
        private static bool MatchesKeywordFilter(
            ...
            string? keyword)
        {
            // Якщо ключового слова немає – не фільтруємо
